...
| 规则 | 说明 |
|------|------|
| **虚拟环境激活** | 运行 Python 前必须激活：`source venv/bin/activate` |
| **提交前测试** | 推送前确保测试通过：`pytest tests/` |
| **临时文件清理** | 调试产生的临时文件测试完成后必须删除 |
...
| **冲突解决策略** | 上游同步冲突时，保留上游重要更新，同时保留本地定制 |
| **安全规则** | 禁止泄露任何 API Key、Token 或环境变量 |

### 四、测试与部署

| 规则 | 说明 |
|------|------|
| **单元测试** | 推送前确保测试通过：`pytest tests/` |
